...
export const action = async ({ request }: ActionArgs) => {
  ...
  const phone2Value = formData.get("customerPhone2") as string;
  const customer = {
  ...
  const invoiceId: number = await db.transaction(async (tx) => {
    ...
    // Generate the next display number for this shop and invoice type
    const displayNumber = await getNextDisplayNumber(user.shopId!, "Invoice");
    ...
    <div className="mx-auto max-w-3xl lg:mt-1.5 p-4">
      ...
      <Form method="post" action="/dashboard/invoices/new">
        <fieldset disabled={transition.state === "submitting"} className="grid gap-4 sm:grid-cols-2">
          ...
          <p className="w-full px-4 sm:pr-0">
            <label htmlFor="customerPhone2" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
              Phone 2 (Optional)
            </label>
            <input
              name="customerPhone2"
